...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: metBalance, fetchStatus: metFetchStatus } = useTokenBalance(tokens.cake.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        ...
        {metFetchStatus !== FetchStatus.Fetched ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(metBalance, 18, 3)}</Text>
        )}
      </Flex>
